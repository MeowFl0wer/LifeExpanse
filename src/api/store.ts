 ...
 *
 * Only `src/api/` may import this. Pages go through the data layer, which is
 * where permission filtering and the ownership rule live — a page that reads
 * the store directly is a page that has to remember those rules itself, and
 * over this project's history that is exactly what kept going wrong.
 *
 * `src/mockData.ts` is the page-facing façade: it re-exports the seed data for
 * modules that have not been migrated yet, and deliberately does **not**
 * re-export anything content-related. Reaching for `allContent` from a page is
 * therefore a compile error rather than something a reviewer has to notice.
 */
import type {
