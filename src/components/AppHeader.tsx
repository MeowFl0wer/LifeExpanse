              ...
              {createOpen && (
                ...
                >
                  {createOptions.map(opt => (
                    <button
                      key={opt.label}
                      className="w-full text-left px-4 py-2.5 text-sm text-[color:var(--foreground)] hover:bg-[color:var(--secondary)] transition-colors flex items-center gap-2"
                      onClick={() => {
                        setCreateOpen(false)
                        alert(`功能提示：「${opt.label}」将跳转到对应的创建页面（前端原型）`)
                      }}
                    >
